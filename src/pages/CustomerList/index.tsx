...
import { VERIFY_TYPE } from '@/constants';
import { useAreaData } from '@/hooks';
import { adminUserLogout, userList } from '@/services';
        ...
        render: (text, record, _, action) => {
          ...
          return (
            <Certify
              ...
              key={enterprise_name}
              status={enterprise_verify_status}
            />
          );
        },
      },
      {
        ...
        render: (text, record, _, action) => [
          <TzButton
            ...
            onClick={() => {
              navigate(
                `/customer/customer-list/customer-info?uid=${record.id}`,
              );
            }}
          >
            查看
          ...
          <TzPopconfirm
            ...
            key={'del'}
            onConfirm={() => {
              adminUserLogout({ uid: record.id }).then((res) => {
